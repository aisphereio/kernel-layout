...
func (s *TodoService) ListTodos(ctx context.Context, req *v1.ListTodosRequest) (*v1.TodoSet, error) {
	...
	set := &v1.TodoSet{Todos: make([]*v1.Todo, 0, len(todos))}
...
func (s *TodoService) SyncTodos(stream v1.TodoService_SyncTodosServer) error {
	for {
		...
		case "update":
			todo, err := s.UpdateTodo(stream.Context(), &v1.UpdateTodoRequest{Todo: req.GetTodo(), UpdateMask: req.GetUpdateMask()})
			if err != nil {
				return err
			}
		...
		case "delete":
			...
			event = &v1.TodoEvent{Action: "deleted", Todo: &v1.Todo{Id: id}, EventTime: timestamppb.Now()}
		default:
...
func convertTodo(in *v1.Todo) *biz.Todo {
	...
	return &biz.Todo{ID: in.GetId(), Title: in.GetTitle(), Content: in.GetContent(), Completed: in.GetCompleted()}
}

func newTodoEvent(action string, todo *biz.Todo) *v1.TodoEvent {
	return &v1.TodoEvent{Action: action, Todo: convertTodoReply(todo), EventTime: timestamppb.New(time.Now())}
}
