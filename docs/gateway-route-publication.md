...
`google.api.http` means an RPC has an HTTP binding. It does not automatically mean the RPC should be published through the public Gateway.

A route is generated into `GatewayManifest` only when the RPC also declares `aisphere.access.v1.policy`.

Use `gateway.publish: DISABLED` for direct-service HTTP routes that must never be published to Gateway.
...
Public Gateway should include product APIs and exclude internal/system/debug/ops routes. Internal service APIs should use a separate internal gateway profile or direct service discovery.
